...
class Synthesis(private val controller: Controller, name: String, typ: Type, private val inputTypes: List[(String, Type)], private val functions: IMap[String, Program], private val objectTypes: IMap[String, List[(String, Type)]], private val printHelpers: PartialFunction[String, Value => String], private val generator: Option[Double => List[(String, Value)]], private val precondition: Option[IMap[String, Value] => Boolean], private val postcondition: Option[(IMap[String, Value], IMap[String, Value], Value) => Boolean], private val objectComparators: Map[String, (Value, Value) => Int]) {
  ...
  private class SolverError(msg: String) extends RuntimeException {
    override def toString: String = "SolverError: " + msg
  }

  /* Checking for equality and other helpers */

  private def yieldEquivalentResults(memoryPre: Memory, cur: Stmt, targetOutput: Value, targetMemoryPost: Memory): Boolean = {
    val (curResult, curMemoryPost) = defaultExecutor.execute(memoryPre, cur)
    areEquivalent(curResult, targetOutput, curMemoryPost, targetMemoryPost)
  }
  private def yieldEquivalentResults(memory: Memory, cur: Stmt, targetStmt: Stmt): Boolean = {
    val (curResult, curMemoryPost) = defaultExecutor.execute(memory, cur)
    val (targetResult, targetMemoryPost) = defaultExecutor.execute(memory, targetStmt)
  ...
  private def hasCorrectForm(evidence: Iterable[Action])(e: Expr): Boolean = {
  ...
  private def possibilitiesToExpr[T <: Stmt](possibilities: Iterable[T], noneFn: => T): T = {
  ...
  private def possibilitiesToStmt(hole: PossibilitiesHole, possibilities: Iterable[Stmt]): Stmt = {
  ...
  private def fillExprHole(expr: Expr, depth: Int = INITIAL_EXPR_DEPTH): Expr = expr match {
    case ExprEvidenceHole(evidence) => holeFiller(expr, evidence, depth, genExpr)
    case _ => expr
  }
  private def fillHoles(stmts: List[Stmt], isPartialTrace: Boolean, depth: Int = INITIAL_EXPR_DEPTH): List[Stmt] = {
	  ...
	  actualCurStmt match {
		...
		def getActionText(isFirstTry: Boolean): (List[Action], Value, Memory) = {
		  ...
		  try {
		    ...
		    if (newPossibilities exists { p => yieldEquivalentResults(memory, p, actionResult, actionMemory) })
		...
		val curStmts = fillHoles(holes, false)
		...
		val (actions, curStmts, newMemory) = unseen match {
		  case _: UnseenStmt => getTrace(controller.getStmtTrace(memory, canFix))
		  case _: UnseenExpr =>
		    val (entered, code, m) = getTrace(controller.getExprTrace(memory, canFix))
		    (List(entered), List(code), m)
		}
  ...
  private def getTraceWithHelpFromUser(code: List[Stmt], inputs: List[(String, Value)], pruneAfterUnseen: Boolean, amFixing: Boolean, otherBranch: Option[(Stmt, UnknownJoinIf)] = None): List[Stmt] = {
	...
	case _ =>
	  ...
	  newCode  // TODO/FIXME: Instead continue with trace?
      }
    }
  }
  ...
  protected[graphprog] def genProgramAndFillHoles(memory: Memory, actions: List[Action], isPartialTrace: Boolean, loops: TMap[Iterate, Loop]): List[Stmt] = {
    ...
    val filledStmts = fillHoles(stmts, isPartialTrace)
    //println(shortPrinter.stringOfStmts(filledStmts))
    filledStmts
  }
  protected[graphprog] def genProgramAndFillHoles(memory: Memory, expr: Expr): Expr = fillExprHole(ExprEvidenceHole(List((expr, memory))))
